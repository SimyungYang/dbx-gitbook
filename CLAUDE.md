...
### 블로그 번역 충실도 규칙 (필수)
`translations/` 폴더의 블로그 번역은 **원본 블로그의 충실한 번역**이어야 합니다. 절대 요약, 재구성, 또는 별도의 글을 작성하지 마세요.

**필수 준수 사항:**
1. **원본 섹션 구조 유지** — 원본 블로그의 모든 섹션과 소제목을 빠짐없이 번역
2. **원본 본문 전체 번역** — 요약하지 말고 모든 단락을 번역 (의역은 허용하되 내용 누락 금지)
3. **원본 이미지 포함** — 원본에 있는 이미지/GIF를 모두 포함 (Databricks CDN URL 직접 사용)
4. **원본에 없는 내용 추가 금지** — "고객 FAQ", "실전 레시피" 등 원본에 없는 섹션을 임의로 추가하지 않음
5. **저자, 날짜 등 메타데이터 보존** — 원문의 저자명, 게시일을 명시

**잘못된 예 — 절대 금지:**
- 원본 7개 섹션 중 1개만 다루고 나머지 누락
- 원본에 없는 "고객 FAQ" 섹션 추가
- 이미지 전부 누락
