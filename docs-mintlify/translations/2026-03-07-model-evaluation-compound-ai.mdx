...
이번 주 초에 우리는 Databricks에서의 새로운 Agent 개발 기능을 발표했습니다. 수백 명의 고객과 대화한 결과, Pilot 단계를 넘어서는 데 두 가지 공통 과제가 있었습니다. 첫째, Production 환경에서 모델 성능에 대한 확신이 부족합니다. 둘째, 반복적으로 개선할 수 있는 명확한 경로가 없습니다. 이 두 가지가 겹치면 프로젝트가 정체되거나, 모델 출력을 수동 평가해 줄 SME(현업 전문가)를 찾느라 시간을 낭비하는 비효율적인 프로세스로 이어집니다.
...
높은 정확도의 내장 LLM Judge 카탈로그를 확장하여, 일상 언어로 작성된 체크리스트나 채점 기준(rubric)을 평가에 활용할 수 있는 **Guidelines AI Judge** (Public Preview)를 소개합니다. *채점 노트(grading notes)* 라고도 불리는 Guidelines는 교사가 채점 기준을 정하는 방식과 유사합니다. 예를 들어 "에세이는 다섯 단락이어야 한다", "각 단락에는 주제문이 있어야 한다" 등의 규칙을 자연어로 정의합니다.
...
Custom Metrics는 내장 메트릭과 LLM Judge를 넘어, AI 애플리케이션에 대한 **커스텀 평가 기준을 직접 정의**할 수 있게 합니다. 입력, 출력, Trace를 비즈니스 요구에 맞게 프로그래밍 방식으로 평가하는 완전한 제어권을 제공합니다. 예를 들어 SQL 생성 Agent의 쿼리가 테스트 DB에서 실제로 실행되는지 검증하는 메트릭을 작성하거나, 내장 Groundedness Judge의 답변-문서 간 일관성 측정 방식을 커스터마이징할 수도 있습니다.
