...
**요약**: Databricks 위에서 외부 지식 베이스(OpenTargets, PubChem, PubMed)를 MCP(Model Context Protocol)를 통해 Databricks의 정형/비정형 데이터와 통합하는 멀티 에이전트 시스템 AiChemy의 구축 방법을 안내합니다. 다양한 AI 에이전트가 자율적으로 협업하여 방대하고 이질적인 데이터셋을 분석하고, 추적 가능한 근거 기반 결과를 제공함으로써 학제 간 신약 개발 연구를 가속화합니다. 연구자들은 질환 타겟 식별, 약물 후보 평가, 상세 특성 정보 검색, 안전성 평가를 수행하여 더 효율적으로 신약을 개발하고 리드(lead) 화합물을 발굴할 수 있습니다.

---

## 멀티 에이전트 시스템이 학제 간 연구를 가속화한다

여러 분야의 전문가 팀처럼 협업하는 멀티 에이전트 AI 시스템이 방대한 데이터셋을 자율적으로 탐색하며 새로운 패턴과 가설을 발견하는 모습을 상상해 보세요. MCP(Model Context Protocol) 덕분에 이것이 현실이 되었습니다. MCP는 다양한 데이터 소스와 도구를 손쉽게 통합하기 위한 새로운 표준으로, 지식 베이스부터 리포트 생성기까지 빠르게 성장하는 MCP 서버 생태계가 무한한 가능성을 열어줍니다.
...
2023년 승인된 경구용 선택적 에스트로겐 수용체 분해제(SERD)인 Elacestrant의 후속 약물을 찾기 위해 화학적 유사성을 활용할 수 있습니다. QSAR(정량적 구조-활성 관계) 원리에 따르면 구조적으로 유사한 분자는 유사한 특성을 공유하므로, Elacestrant와 구조가 유사한 약물 유사(drug-like) 분자를 대규모 ZINC15 화학 라이브러리에서 검색합니다. 구체적으로, Elacestrant의 1024비트 ECFP(Extended-Connectivity Fingerprint) 분자 임베딩을 쿼리 벡터로 사용하여 Databricks Vector Search로 ZINC의 25만 분자 인덱스에서 가장 유사한 분자를 찾습니다.
