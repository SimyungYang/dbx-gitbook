...
이런 머신 규모의 공격에 맞서, 최고의 보안 운영 팀조차 구조적 한계에 부딪힙니다. 현재의 보안 도구는 애널리스트가 수동으로 알림을 보강하고, 탐지 규칙을 직접 작성하며, 위협 헌팅 가설을 수일에서 수주에 걸쳐 검증해야 합니다. 이런 워크플로우는 사람 속도의 위협에는 통했지만, 24시간 머신 속도로 가동되는 AI 기반 공격 앞에서는 아키텍처 자체가 병목이 됩니다. [ZeroDayClock.com](https://zerodayclock.com/)에 따르면, 익스플로잇까지의 평균 시간(Mean Time to Exploit)은 2025년 23.2일에서 2026년에는 불과 1.6일로 급격히 단축되었습니다.

데이터 측면에서 보면 문제는 더 심각합니다. 대규모 엔터프라이즈는 매일 TB~PB 규모의 보안 데이터를 생성합니다. 그러나 전통적 SIEM은 스토리지와 컴퓨트가 결합되어 있어, 데이터를 수집할수록 비용이 늘어납니다. 결국 팀은 데이터 수집을 제한하고, 과거 데이터를 삭제하며, 채팅 로그나 비디오 같은 멀티모달 소스를 아예 무시합니다.

이로 인해 위험한 비대칭이 발생합니다. 공격자는 AI 에이전트로 모든 것을 분석하고 어디서든 공격하는 반면, 방어자는 자기 데이터의 일부만 볼 수 있습니다. 전통적인 SIEM은 멀티모달 데이터를 처리하지 못하지만, 바로 그곳에 소셜 엔지니어링(Social Engineering) 공격, 내부자 위협, 프롬프트 인젝션(Prompt Injection) 시도가 숨어 있습니다.

이것은 단순한 비용이나 규모의 문제가 아닙니다. 위협의 수준과 방어 도구 사이의 근본적인 아키텍처 불일치입니다. 비슷한 문제를 이전에도 겪은 적이 있습니다. 데이터 웨어하우스도 비싼 수집 비용, 사일로화된 데이터, 제한된 활용이라는 동일한 한계를 가지고 있었습니다. 레이크하우스가 오픈 포맷, 저렴한 스토리지, 모든 데이터 유형 지원으로 그 패러다임을 바꿨습니다. 이제 같은 변혁을 보안 영역에 가져옵니다.
