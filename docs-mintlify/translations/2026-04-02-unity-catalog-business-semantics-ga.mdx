...
데이터와 AI가 기업의 핵심이 되면서, 비즈니스 개념에 대한 일관된 이해가 필수가 되었습니다. 분석가, 엔지니어, 경영진, 그리고 이제는 AI 에이전트까지 동일한 데이터를 서로 다르게 해석하는 경우가 많아, 메트릭 불일치(metric drift), 상충하는 보고서, 신뢰 저하로 이어집니다. 오랫동안 이러한 비즈니스 개념들은 BI 도구와 대시보드 안에 갇혀 있었습니다. 에이전틱 AI 시대에 에이전트가 데이터를 추론하고 자율적으로 행동하게 되면, 파편화된 정의는 단순히 혼란을 일으키는 데 그치지 않고 그 혼란을 증폭시킵니다.
...
- **독점적이고 파편화됨**: 멀티 도구, 멀티 에이전트 환경에서 각 BI 모델은 자체 언어로 동작합니다. 정의가 대시보드, 모델, 스프레드시트 안에 갇혀 조직 전반의 거버넌스, 접근 정책 집행, 리니지 추적이 거의 불가능해집니다.
- **표현 계층에 묻힌 정의**: 시맨틱 레이어가 데이터 기반이 아닌 표현 계층(presentation tier)에 위치하기 때문에, 팀들은 대시보드와 보고서마다 같은 메트릭을 반복 정의합니다. 이 방식은 시맨틱스를 취약하고, 일관성 없고, 확장하기 어렵게 만듭니다.
- **AI에 부적합**: 기존 레이어는 무거운 사전 모델링에 의존해, 빠르게 변하는 비즈니스 질문이나 AI 에이전트의 자유로운 프롬프트에 대응하지 못합니다. 변경 시마다 전문가 개입이 필요해 대응 속도가 저하되고 신뢰가 훼손됩니다.

이러한 한계는 오랫동안 데이터 및 AI 팀을 좌절시켜 왔습니다. 민첩성과 신뢰할 수 있는 답변이 타협 불가인 오늘날 AI 중심 환경에서, 이는 발전을 가로막는 심각한 장벽입니다.

## Unity Catalog Business Semantics: 통합·오픈 접근 방식

Unity Catalog Business Semantics는 시맨틱스를 Databricks Data Intelligence Platform의 핵심에서 통합 관리하겠다는 근본적인 전환입니다. Unity Catalog에 직접 내장되어, 기존에 의존하던 거버넌스, 보안, 리니지를 확장하면서 이 정의들을 업무 현장 어디서든 활용할 수 있게 합니다.
