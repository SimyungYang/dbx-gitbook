...
**Databricks 데이터 인텔리전스 플랫폼과 결합하면, IoT, 클릭스트림, 애플리케이션 텔레메트리 등의 이벤트 데이터가 실시간 분석과 AI를 구동합니다.** 전통적으로 이 데이터를 수집하려면 소스와 레이크하우스 사이에 메시지 버스, Spark 작업 등 여러 단계를 거쳐야 했습니다. 운영 부담이 커지고, 데이터가 중복 저장되며, 전문 기술이 필요합니다. 레이크하우스가 유일한 목적지라면 특히 비효율적입니다.

데이터가 레이크하우스에 적재되면 다운스트림 분석을 위해 변환/정제됩니다. 그런데 팀은 이 분석 데이터를 운영 사용 사례에도 활용해야 하는 경우가 많으며, 이를 위한 커스텀 앱 구축에는 상당한 노력이 필요합니다. 전용 OLTP 데이터베이스 인스턴스(네트워킹, 모니터링, 백업 포함) 프로비저닝과 유지보수가 필요합니다. 또한 분석 데이터를 DB로 역전송하는 [Reverse ETL](https://www.databricks.com/blog/reverse-etl-lakebase-activate-your-lakehouse-data-operational-analytics) 프로세스 관리도 해야 합니다. 이런 추가 파이프라인들은 인프라 복잡성을 높이고, 정작 핵심 목표인 비즈니스 앱 구축에서 주의를 분산시킵니다.
