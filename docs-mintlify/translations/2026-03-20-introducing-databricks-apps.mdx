...
Databricks Apps의 핵심 장점 중 하나는 SQL 대신 코드를 사용하여 비기술 사용자를 위한 데이터 애플리케이션을 만들 수 있다는 점입니다. 이를 통해 조직 내 더 넓은 구성원에게 복잡한 데이터 인사이트를 전달할 수 있습니다. 예를 들어, 마케팅 팀이 캠페인 성과 지표를 시각화하는 맞춤형 대시보드를 만들면, 기술적 배경이 없는 팀원들도 데이터를 쉽게 해석하고 행동에 옮길 수 있습니다.

나아가 Databricks Apps는 AI 컴포넌트를 통합할 수 있어, 필요에 따라 개발자가 특정 AI 모델을 호출할 수 있습니다. 예를 들어, 고객 피드백에 대한 감성 분석이나 매출 예측을 위한 예측 모델링 같은 고도화된 기능을 앱에 내장할 수 있어, 비기술 사용자가 얻는 데이터 인사이트의 가치를 한층 높여줍니다.

한번 빌드된 앱은 Databricks 내에서 완전히 배포되고 관리되므로, 팀이 인프라를 직접 구성하고 관리할 필요가 없습니다. Unity Catalog에 이미 구성된 데이터 접근 제어를 그대로 따르며, 동일한 통합 거버넌스 모델로 사용자별 배포 권한을 제어합니다. Databricks Apps를 통해 조직은 Databricks 환경 내에서 원활하게 실행되는 커스텀 애플리케이션을 만들어 데이터 및 AI 투자의 잠재력을 극대화할 수 있습니다.

---

## 데이터 애플리케이션 구축의 어려움

데이터 중심 시대에 조직들은 데이터 자산에서 더 많은 가치를 끌어내려 하고 있습니다. 하지만 내부 데이터 애플리케이션을 구축하고 배포하는 일은 전통적으로 복잡하고 시간이 많이 걸렸습니다.

- 개발자들은 앱 개발 대신 인프라 관리에 시간을 빼앗기게 됩니다.
- 데이터 거버넌스와 컴플라이언스를 위해 접근 제어를 수동으로 구현해야 합니다.
- 앱 공유와 권한 관리가 다른 데이터 자산과 분리되어, 거버넌스가 파편화됩니다.
