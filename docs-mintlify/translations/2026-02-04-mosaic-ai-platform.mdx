...
지난 한 해 동안, 일반 지식 태스크에서 강력한 추론 능력을 보여주는 상용 및 오픈소스 파운데이션 모델(foundation model)이 빠르게 늘어났습니다. 범용 모델은 중요한 구성 요소이지만, 프로덕션 AI 애플리케이션은 종종 **Compound AI System**(복합 AI 시스템)을 채택합니다. 이는 파인튜닝된 모델, 검색(retrieval), 툴 사용(tool use), 추론 에이전트(reasoning agents) 등 여러 컴포넌트를 결합한 시스템입니다.

이러한 **AI 에이전트 시스템** 은 파운데이션 모델을 강화하여 훨씬 높은 품질을 달성하고, 고객이 GenAI 앱을 프로덕션에 자신 있게 배포할 수 있도록 도와줍니다.
...
단일 모놀리식(monolithic) AI 모델에서 복합 시스템으로의 전환은 학계와 산업계 모두에서 활발히 연구되는 분야입니다. 최근 연구에 따르면, 최첨단 AI 성과는 단일 모델이 아니라 여러 컴포넌트로 구성된 복합 시스템에서 점점 더 많이 나오고 있습니다.

이러한 추세는 실제 고객 사례에서도 확인됩니다. 금융 리서치 기업 FactSet은 상용 LLM을 Text-to-Financial-Formula 사용 사례에 배포했을 때 수식 정확도가 55%에 불과했지만, 모델을 복합 시스템으로 모듈화하여 각 태스크를 전문화함으로써 정확도를 85%까지 끌어올렸습니다. Databricks Mosaic AI는 다음과 같은 제품들을 통해 AI 시스템 구축을 지원합니다.
