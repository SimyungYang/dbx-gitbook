...
LLM(대규모 언어 모델)이 이 코드를 더 빠르게 생성해 주긴 하지만, 올바르게 만들고 시간이 지나도 정확성을 유지해야 하는 복잡성 자체는 줄어들지 않습니다. 코드를 생성하는 것과 데이터의 맥락을 이해하는 것은 별개이기 때문입니다.
...
이 패턴을 잘 아는 팀이라도, 처음부터 올바르게 구현하고 시간이 지나도 정확성을 유지하기란 쉽지 않습니다. 데이터 볼륨이 증가하고 유스케이스가 확장될수록 파이프라인은 취약해지고, 정확성 문제는 뒤늦게 드러납니다. 작은 변경 하나에도 다운스트림 테이블이 깨지지 않도록 신중하게 코드를 재작성해야 합니다.
