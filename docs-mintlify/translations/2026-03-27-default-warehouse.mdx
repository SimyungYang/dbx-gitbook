...
**Default Warehouse** 가 이제 Databricks SQL에서 정식 출시(GA)되었습니다. 관리자가 임시(ad hoc) 환경에서 자동 선택될 SQL 웨어하우스를 지정할 수 있는 기능입니다. 사용자가 어떤 웨어하우스를 선택해야 하는지 몰라도, 탐색 쿼리가 올바른 컴퓨팅에서 적절한 비용으로 실행됩니다.

그 결과, 의도치 않은 웨어하우스 기동이 줄어들고, 워크로드 격리가 강화되며, 성능과 비용을 예측할 수 있게 됩니다.
...
설정 가능한 기본값이 없으면 시스템은 "마지막에 선택한 웨어하우스(Last Selected)"를 재사용하거나 알파벳 순으로 폴백(fallback)합니다. 이로 인해 다음과 같은 문제가 발생합니다:

- **성능 저하** — 임시 쿼리가 대형 프로덕션 웨어하우스로 유입되어 핵심 워크로드와 리소스 경합이 발생합니다
