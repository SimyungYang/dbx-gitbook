...
현대 기업에서 비즈니스 의사결정자가 데이터 인사이트를 원할 때, 보통 두 가지 방법 중 하나를 택합니다. 분석가에게 맞춤형 리포트를 요청하거나(느리고 병목이 생기기 쉬움), 직접 대시보드를 탐색하는 것입니다(복잡하고 원하는 답을 찾기 어려운 경우가 많음).

AI/BI Genie는 이 문제를 해결합니다. 분석가가 특정 주제(예: 영업 파이프라인)에 집중한 Genie Space를 구성하면, 데이터(테이블, 뷰 등)와 시맨틱 정보(메트릭 정의, 샘플 쿼리, 텍스트 지시사항, 인증된 자산 등)가 함께 패키징되어 자연어 질의가 가능해집니다.
