...
**Databricks**는 오늘 **Lakewatch**를 발표했습니다. Lakewatch는 점점 정교해지는 AI 기반 공격자로부터 조직을 방어하도록 설계된 새로운 오픈형 에이전틱 SIEM(Security Information and Event Management)입니다.

Lakewatch는 보안, IT, 비즈니스 데이터를 단일 거버넌스 환경으로 통합하여 AI 기반 탐지 및 대응을 가능하게 합니다. 오픈 포맷과 오픈 에코시스템을 통해 전례 없는 규모의 멀티모달 데이터를 수집, 보존, 분석하면서도 비용을 절감하고 벤더 종속(Vendor Lock-in)을 제거합니다. 보안 팀은 기업 전반의 완전한 가시성을 확보하고, 방어용 보안 에이전트를 배포하여 위협 탐지와 대응을 대규모로 자동화할 수 있습니다. 현재 **Private Preview**로 제공됩니다.

## 머신 속도로 방어하기

AI 위협은 인간 주도 방어가 따라잡기 어려운 속도와 복잡성으로 진화하고 있습니다. 공격자는 에이전트를 배포해 시스템을 지속적으로 스캔하고, 취약점을 발견하며, 머신 속도로 조율된 공격을 실행합니다. 반면 방어자는 불완전한 데이터, 수동 워크플로우, 사일로화된 아키텍처에 여전히 발목이 잡혀 있습니다. 높은 수집 비용 탓에 데이터의 최대 75%를 버려야 하는 상황입니다.

이로 인해 위험한 비대칭이 발생합니다. 공격자는 AI 에이전트로 어디서든 공격을 감행하지만, 방어자는 전체 데이터의 극히 일부만 볼 수 있고 팀의 반응 속도에 의해 제한됩니다.

Lakewatch는 이 격차를 해소합니다. 모든 데이터를 오픈 포맷으로 통합하여, 데이터를 이동하거나 복제하지 않고도 수년치 데이터를 비용 효율적으로 분석할 수 있습니다. 소셜 엔지니어링, 내부자 위협, 이상 징후를 식별하기 위한 영상, 음성 등 멀티모달 데이터도 포함됩니다. AI 에이전트 군집이 탐지, 트리아지(Triage, 우선순위 분류), 위협 헌팅을 자동화하여, 머신 속도의 공격에 머신 속도의 방어로 맞섭니다.
