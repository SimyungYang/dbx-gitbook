...
> **참고**
Spark Declarative Pipelines(SDP)의 기본 개념, Streaming Table과 Materialized View의 차이는 **[교육자료 → 데이터 엔지니어링 → SDP](/intro/data-engineering/spark-declarative-pipelines/overview)** 를 참고하세요. 이 가이드는 프로덕션 운영 시 Full Refresh 방지 전략에 집중합니다.
