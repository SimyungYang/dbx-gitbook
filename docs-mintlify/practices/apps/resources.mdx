...
이 중에서 실무에서 가장 자주 사용하는 리소스는 **SQL Warehouse**, **Serving Endpoint**, **Secret**, **Lakebase (postgres)** 네 가지입니다. 특히 Lakebase는 앱의 영속 저장소로 사용할 때 매우 유용합니다. 자세한 Lakebase 연동 가이드는 [고급 기능](/practices/apps/advanced) 페이지를 참고하세요.
