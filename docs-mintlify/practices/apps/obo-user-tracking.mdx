...
> **고객 질문**: "외부 포털에서 SP 1개로 Databricks를 호출하는데, 실제 사용자를 식별할 수 없습니다. SP 1개를 유지하면서 호출한 사용자를 특정할 수 있나요?"
...
```
[OBO] 외부 포털 → 사용자 SSO 로그인 → OBO 토큰 → Databricks
...
```sql
-- 외부 포털 서버에서 Databricks SQL API 호출 시
SET spark.databricks.queryTag = '{"user": "hong.gildong@company.com", "source": "MyPortal", "department": "marketing"}';
