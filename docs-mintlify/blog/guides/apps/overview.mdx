...
> **참고**
Databricks Apps의 기본 개념(프레임워크 소개, 아키텍처 기초, 시작하기)은 **[교육자료 → 개발 도구 → Databricks Apps](/training/13-appendix/databricks-apps)** 를 참고하세요. 이 가이드는 실전 배포와 운영 팁에 집중합니다.
...
앱 생성 시 Databricks가 **전용 서비스 프린시펄(SP)** 을 자동으로 생성합니다. 이 SP가 앱의 영구적인 아이덴티티로 동작하며, UC 테이블, SQL Warehouse 등에 대한 접근 권한을 이 SP에 부여해야 합니다.

> **참고**
SP 자동 생성 메커니즘, 권한 관리 실전 패턴(UC 그룹, 자동화 스크립트, app.yaml 리소스 선언)에 대한 상세 내용은 **[서비스 프린시펄 (SP) 권한 관리](/blog/guides/apps/service-principals)** 페이지를 참고하세요.
