...
| 프로토콜 | 방향 | 연결 대상 | 역할 |
|---------|------|----------|------|
| **A2A** (Agent-to-Agent) | 수평 | Agent A ↔ Agent B | Agent끼리 협업 |
| **MCP** (Model Context Protocol) | 수직 | Agent → Tool/DB, API/SaaS | Agent가 Tool을 사용 |
...
| [AWS, Microsoft & Meta](aws-others-basics) | Bedrock Agents, AgentCore, Copilot Studio, Llama 4 |
| [Databricks](databricks) | Agent Bricks, TAO/ALHF, Supervisor Agent, Gateway, MLflow 3.0 |
| [멀티에이전트 트렌드](multi-agent-trends-basics) | 오케스트레이션 패턴, Observability, 메모리, 안전성, Vibe Coding |
