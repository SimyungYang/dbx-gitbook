...
| 계층 | 구성 요소 | 역할 |
|------|----------|------|
| **Model Serving** | Supervisor Agent Endpoint | 모든 요청을 수신하고 서브 에이전트로 라우팅 |
| **서브 에이전트** | KA Endpoint | Knowledge Assistant 문서 Q&A |
| **서브 에이전트** | Genie Space (API) | 자연어 → SQL 데이터 탐색 |
| **데이터 소스** | Vector Search Index | KA용 문서 임베딩 검색 |
| **데이터 소스** | SQL Warehouse (UC Tables) | Genie용 Gold 테이블 쿼리 |
| **도구** | UC Functions | 재사용 가능한 분석 함수 |
| **도구** | MCP Servers | 외부 도구 연동 |
| **관찰성** | MLflow Monitoring | 에이전트 실행 추적 및 품질 모니터링 |
