...
```mermaid
flowchart TD
    A[정형 데이터<br/>AI4I 2020] --> C[Feature Engineering<br/>Spark/Pandas]
    B[비정형 데이터<br/>MVTec AD] --> D[Image Processing<br/>Anomalib]
    
    C --> E[XGBoost Train<br/>+ SHAP 해석]
    D --> F[PatchCore<br/>이상탐지 학습]
    
    E --> G[UC Model Registry<br/>Champion / Challenger]
    F --> G
    
    G --> H[Batch Predict<br/>일 4회]
    G --> I[Model Serve<br/>실시간]
    
    H --> J[Lakehouse Monitor<br/>드리프트 탐지]
    I --> J
    
    J --> K[MLOps Agent + Workflows<br/>주 1회 재학습]
    K --> E
    K --> F
