...
<Steps>
  <Step title="Event Generator (DB App)">
    Databricks App이 주기적으로 이벤트 데이터를 생성합니다.
  </Step>
  <Step title="UC Volume (/landing/)">
    생성된 `*.json` 파일이 UC Volume에 저장됩니다.
  </Step>
  <Step title="Auto Loader (cloudFiles)">
    새 파일을 자동 감지하여 스트림으로 수집합니다.
  </Step>
  <Step title="SDP Pipeline (Bronze → Silver)">
    증분 처리로 데이터를 정제하고 검증합니다.
  </Step>
  <Step title="Gold Materialized View">
    집계가 자동 갱신되어 대시보드에 반영됩니다.
  </Step>
</Steps>
