...
| 계층 | 구성 요소 | 역할 |
|------|----------|------|
| **Supervisor Agent (MAS)** | "스마트TV 통합 어시스턴트" | 사용자 질문을 적절한 서브 에이전트로 라우팅 |
| **서브 에이전트 1** | Knowledge Assistant | TV 사용 가이드 Q&A (문서 RAG) |
| **서브 에이전트 2** | Genie Space | SQL 기반 데이터 탐색 (Gold 테이블) |
| **서브 에이전트 3** | 추천 Endpoint | Vector Search + LLM 추천 (개인화) |
| **서브 에이전트 4** | 광고 성과 분석 함수 | UC Function 광고 KPI 자동 계산 (SQL 함수) |
| **플랫폼** | Databricks Platform | Unity Catalog, Vector Search, Model Serving, MLflow |
...
<Steps>
  <Step title="소스 테이블 (Delta Table)">
    `content_metadata` 테이블의 `description` 컬럼이 임베딩 대상입니다.
  </Step>
  <Step title="Delta Sync → Vector Index">
    소스 테이블이 변경되면 자동으로 동기화되어 Embedding Vectors(`[0.1, 0.3, ..., 0.7, 0.2]`)로 변환됩니다.
  </Step>
  <Step title="Similarity Search">
    사용자 쿼리(예: "로맨스 드라마 추천")를 벡터로 변환하여 유사도 검색을 수행합니다.
  </Step>
  <Step title="Top-K 유사 콘텐츠 반환">
    가장 유사한 콘텐츠 K개가 결과로 반환됩니다.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Instructed Retriever (검색)">
    UC Volume Documents(5개 가이드)에서 질문과 관련된 문서 청크를 검색합니다.
  </Step>
  <Step title="LLM 답변 생성 (문서 기반)">
    검색된 문서를 기반으로 LLM이 답변을 생성합니다.
  </Step>
  <Step title="답변 반환">
    "FastTV에서는 뉴스, 스포츠, 영화 등 100개 이상의 무료 채널을 제공합니다..."
  </Step>
</Steps>
...
<Steps>
  <Step title="사용자 자연어 질문">
    "이번 달 인기 콘텐츠 Top 10은?"
  </Step>
  <Step title="NL → SQL 변환">
    자연어를 분석하여 Gold Tables(Unity Catalog)에 대한 SQL 쿼리로 변환합니다.
  </Step>
  <Step title="SQL 실행">
    ```sql
  ...
  <Step title="결과 + 해석">
    "Netflix가 1위, YouTube가 2위..."와 같이 비즈니스 관점에서 결과를 해석하여 제공합니다.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Router (라우팅 엔진)">
    질문 의도를 분석하여 적절한 서브 에이전트로 라우팅합니다.

    | 질문 유형 | 라우팅 대상 |
    |----------|-----------|
    | 광고 성과 | UC Function (광고분석) |
    | 사용법 | Knowledge Assistant |
    | 데이터 | Genie Space |
    | 추천 | Endpoint (추천) |
  </Step>
  <Step title="서브 에이전트 실행">
    라우팅된 서브 에이전트(KA, Genie Space, UC Function, Endpoint)가 질문을 처리합니다. 필요시 여러 서브 에이전트를 조합합니다.
  </Step>
  <Step title="통합 응답">
    "쿠팡의 2월 광고 성과: CTR 3.2%, 노출 15,432건, CPC $0.02, 전월 대비 CTR 15% 상승..."
  </Step>
</Steps>
