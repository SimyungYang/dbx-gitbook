...
> Lakebase(PostgreSQL OLTP DB)를 활용한 운영 데이터 저장소를 구축합니다.
...
- **Lakebase**: PostgreSQL OLTP 데이터베이스 생성 및 활용
...
| **기술** | FastAPI, Gradio, Streamlit | PostgreSQL |
...
```
Lakebase(PostgreSQL 데이터베이스) 인스턴스를 만들어줘.

인스턴스 이름: smarttv-demo-lakebase
카탈로그: {catalog}

Lakebase 설명:
- Databricks에서 제공하는 PostgreSQL OLTP 데이터베이스
...
| 계층 | 구성 요소 | 역할 | 데이터 흐름 |
|------|----------|------|-----------|
| **분석 저장소** | Delta Lake | Bronze/Silver/Gold Tables, ML 피처 | Gold Tables → Lakebase로 캐싱 |
| **운영 저장소** | Lakebase | 추천 캐시, 사용자 설정, 피드백 | Apps에서 읽기/쓰기 |
| **AI 서빙** | Model Serving | ALS 모델, Agent Endpoint | 추론 결과 → Lakebase 캐시 |
| **앱 (Gradio)** | 챗봇 App | AI 어시스턴트 채팅 UI | Agent Endpoint 호출 |
| **앱 (Streamlit)** | 대시보드 App | 데이터 탐색 시각화 | SQL Warehouse 쿼리 |
| **앱 (FastAPI)** | TV 포탈 App | REST API 서버 | Lakebase 읽기/쓰기 |
...
| **Lakebase** | PostgreSQL OLTP DB, 테이블 생성, CRUD |
