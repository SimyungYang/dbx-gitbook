...
| 계층 | 구성 요소 | 역할 |
|------|----------|------|
| **AI 코딩 어시스턴트** | Claude Code / Cursor / Windsurf | 사용자와 대화하며 코드 생성 및 실행 |
| **Skills (마크다운)** | 패턴, 베스트 프랙티스, 코드 규약 | AI에게 Databricks 패턴/규약을 가르침 |
| **MCP Server (50+ 실행 도구)** | SQL 실행, 잡 생성/관리, 파이프라인 관리, 앱 배포, 카탈로그 관리, MLflow 연동, 벡터 검색, Agent Bricks | AI가 Databricks 작업을 직접 실행 |
| **databricks-tools-core** | 공유 Python 라이브러리 | LangChain/OpenAI 등과 통합 |
| **Databricks Workspace** | Unity Catalog, SQL Warehouse, Compute | 실제 데이터 플랫폼 |
