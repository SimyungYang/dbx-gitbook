...
> **사전 지식이 필요합니다**
- [Medallion 아키텍처](/intro/lakehouse/medallion-architecture) — Bronze(원본) → Silver(정제) → Gold(집계) 데이터 계층
- [SDP(Spark Declarative Pipelines)](/intro/data-engineering/spark-declarative-pipelines/overview) — 선언적 파이프라인 프레임워크
- [Lakeflow Jobs](/intro/data-engineering/lakeflow-jobs/overview) — 워크플로우 오케스트레이션 도구
- [Lakeflow Connect](/intro/data-engineering/lakeflow-connect/overview) — 외부 DB 데이터 수집
...
| **CDC** | Change Data Capture. 외부 DB에서 변경된 데이터만 캡처하여 수집 ([상세](/intro/data-engineering/lakeflow-connect/overview)) |
...
| **Materialized View** | 쿼리 결과를 물리적으로 저장하는 뷰. 소스 변경 시 자동으로 증분 갱신 ([상세](/intro/data-warehousing/materialized-views)) |
...
[Lakeflow Jobs](/intro/data-engineering/lakeflow-jobs/overview)는 여러 종류의 태스크를 **DAG로 조합**하여 전체 파이프라인을 하나의 워크플로로 관리합니다. 수집 → 변환 → 품질 검증 → 서빙까지 하나의 Job으로 구성할 수 있습니다.
...
**이 가이드의 기초 문서:**
- [Medallion 아키텍처](/intro/lakehouse/medallion-architecture) — Bronze/Silver/Gold 설계 패턴
- [SDP 개요](/intro/data-engineering/spark-declarative-pipelines/overview) — 선언적 파이프라인
- [SDP CDC와 SCD](/intro/data-engineering/spark-declarative-pipelines/cdc-and-scd) — CDC 처리 상세
- [Lakeflow Jobs](/intro/data-engineering/lakeflow-jobs/overview) — 워크플로우 오케스트레이션
- [Lakeflow Connect](/intro/data-engineering/lakeflow-connect/overview) — 외부 DB 수집

**심화 문서:**
- [Structured Streaming](/intro/data-engineering/structured-streaming) — `foreachBatch`, 체크포인트 상세
- [Lakeflow 증분 처리](/practices/lakeflow-incremental/overview) — Full Refresh 방지 전략
