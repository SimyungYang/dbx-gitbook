...
> 💡 **Lakebase** 는 Databricks가 제공하는 **완전 관리형 클라우드 네이티브 PostgreSQL 데이터베이스**로, 레이크하우스에 OLTP 역량을 더합니다. 레이크하우스 플랫폼 안에서 OLTP(트랜잭션 처리)와 OLAP(분석)를 하나로 통합하여, 데이터 복사 없이 운영 데이터를 바로 분석에 활용할 수 있게 합니다.
...
| **Lakebase (PostgreSQL)** | OLTP 데이터 저장 | PostgreSQL 데이터베이스입니다 |
