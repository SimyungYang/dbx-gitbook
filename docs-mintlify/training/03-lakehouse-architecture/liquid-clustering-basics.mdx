...
|---|---|---|
| **데이터 배치** | 컬럼 값 기준으로 고정된 디렉토리 분할 | 컬럼 값 기반으로 데이터 파일이 유동적으로 재배치 |
| **클러스터링 키 변경** | 테이블 재생성 필요 | `ALTER TABLE ... CLUSTER BY` 로 즉시 변경 |
| **편향(skew) 대응** | 수동 리파티셔닝 필요 | 증분 클러스터링이 자동 보정 |
| **운영 부담** | OPTIMIZE 실행 시마다 ZORDER 컬럼 재지정 | Predictive Optimization 이 자동 실행 |
| **소량 파일 문제** | 파티션 폭증 시 성능 저하 | 자동 파일 컴팩션 |

> 참고: [Databricks: Use Liquid Clustering for Delta tables](https://docs.databricks.com/aws/en/delta/clustering.html)
