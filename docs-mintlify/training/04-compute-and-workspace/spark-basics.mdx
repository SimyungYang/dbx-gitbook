...
![Spark 클러스터 아키텍처 — Driver와 Executor](https://spark.apache.org/docs/latest/img/cluster-overview.png)
...
df = spark.createDataFrame([
    ("Alice", 28, "Seoul",  4500000),
    ("Bob",   34, "Busan",  5200000),
    ("Carol", 25, "Daegu",  3800000),
...
filtered  = df.filter(df.age >= 30)                    # 30세 이상 필터링
selected  = filtered.select("name", "salary")          # 필요한 컬럼만 선택
sorted_df = selected.orderBy("salary", ascending=False)  # 연봉 내림차순 정렬
```
...
# 3. 변환: 새 컬럼 추가 (주문 금액 기준으로 사이즈 라벨링)
...
(summary.write
    .format("delta")
    .mode("overwrite")               # 기존 데이터 덮어쓰기 (append/ignore/errorifexists 와 택1)
...
# - broadcast(df) hint 는 임계값을 무시하고 강제로 broadcast 시도
# - 단, 실제 크기가 Driver memory 의 일정 비율을 넘으면 fallback 됨
result = large_orders.join(
    broadcast(small_stores),  # small_stores 전체를 모든 Executor 로 복제
    "store_id"
)

# spark.sql.autoBroadcastJoinThreshold
#   - 의미: 이 크기 미만의 테이블은 자동으로 Broadcast Join 후보로 선택
#   - 기본값: 10MB (10 * 1024 * 1024 = 10485760)
#   - 단위: bytes
#   - 비활성화: -1 (자동 broadcast 사용 안 함)
#   - 튜닝: Driver 메모리가 충분하고 dim 테이블이 큰 경우 100MB ~ 500MB 까지 상향
...
#   - 의미: GROUP BY / JOIN / ORDER BY 등 셔플 후 생성될 파티션 수
#   - 기본값: 200 (대부분의 워크로드에 부적합)
#   - 영향: 너무 적으면 OOM/GC, 너무 많으면 스케줄링 오버헤드·소형 파일
#   - Databricks 권장: AQE 활성화 + "auto" 로 두면 런타임에 자동 조정
spark.conf.get("spark.sql.shuffle.partitions")  # "200"

# 데이터 규모에 맞게 조정
# 일반 가이드: 셔플 후 각 파티션이 100~200MB 가 되도록 설정
...
```python
# AQE 관련 설정 (Databricks 에서는 기본 활성화)

# spark.sql.adaptive.enabled
#   - AQE 마스터 스위치. true 일 때 아래 세부 기능이 동작
#   - 기본값: true (Databricks) / false (OSS Spark < 3.2)
spark.conf.set("spark.sql.adaptive.enabled", "true")

# spark.sql.adaptive.coalescePartitions.enabled
#   - 셔플 후 작은 파티션을 자동 병합 (Coalescing Post-Shuffle Partitions)
#   - 기본값: true
...
# Skew Join 감지 임계값
#   - skewedPartitionThresholdInBytes: 이 크기 이상이면 skew 후보 (기본 256MB)
#   - skewedPartitionFactor:           파티션 중앙값의 N 배 이상이면 skew (기본 5)
#   - 두 조건을 동시에 만족할 때 분할
spark.conf.set("spark.sql.adaptive.skewJoin.skewedPartitionThresholdInBytes", str(256 * 1024 * 1024))
spark.conf.set("spark.sql.adaptive.skewJoin.skewedPartitionFactor", "5")

# spark.sql.adaptive.advisoryPartitionSizeInBytes
#   - 셔플 파티션을 병합/분할할 때 목표 크기
#   - 기본값: 64MB
#   - 튜닝: 클러스터 코어가 많을수록 키워서 task 수 줄임 (128~256MB 권장)
spark.conf.set("spark.sql.adaptive.advisoryPartitionSizeInBytes", str(128 * 1024 * 1024))  # 128MB
```
...
# AQE 로 셔플 파티션·skew 자동 보정 → 가장 먼저 켜기
spark.conf.set("spark.sql.adaptive.enabled", "true")

# spark.sql.shuffle.partitions = "auto"
#   - Databricks 전용 값. AQE 가 클러스터 코어/데이터 크기 기반으로 동적 결정
#   - OSS Spark 에서는 정수 값만 허용
spark.conf.set("spark.sql.shuffle.partitions", "auto")

# Executor 별 메모리 사용량 스냅샷
#   - getExecutorMemoryStatus() : {executor_id: (max_memory, remaining_memory)}
#   - private API 이므로 운영 코드보다는 디버깅에서만 사용
