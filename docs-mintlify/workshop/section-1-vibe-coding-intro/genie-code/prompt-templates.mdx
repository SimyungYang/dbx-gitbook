...
> 💡 **완성된 예시**: `Bronze → Silver → Gold 변환을 SDP로 만들어줘. 파이프라인 이름: lge_smart_tv_pipeline, 타겟 카탈로그: smart_tv. Bronze→Silver는 Streaming Table, Silver→Gold는 Materialized View. 각 테이블에 Expectations 추가.`
...
> 💡 **완성된 예시**: `Knowledge Assistant를 만들어줘. 이름: Smart TV 가이드. 지식 소스: @smart_tv.bronze.tv_knowledge_base. Vector Search 엔드포인트: vs-endpoint-smart-tv. 임베딩 모델: databricks-gte-large-en. LLM: databricks-claude-sonnet-4. System Prompt: "Smart TV OS 전문 어시스턴트입니다. 제공된 문서에 기반하여 한국어로 답변하세요."`
