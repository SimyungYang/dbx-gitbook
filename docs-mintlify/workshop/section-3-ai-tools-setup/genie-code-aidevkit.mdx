...
# 2. 간편 배포 실행 (앱 생성 + 배포 + 권한 + Skills 자동)
./deploy.sh
```

> 가전 IoT 카탈로그를 사용하는 경우: `./deploy.sh --catalog lge_appliance`

deploy.sh가 자동으로 처리하는 것:
1. MCP 앱 생성 (`mcp-ai-dev-kit`)
2. 소스 코드 업로드 및 배포
3. 서비스 프린시펄(SP) 권한 자동 부여 (Workspace, SQL Warehouse)
4. AI Dev Kit Skills를 Workspace에 배포
5. 카탈로그 GRANT SQL 출력 (노트북에서 실행 필요)

배포 완료 후:
1. 스크립트가 출력한 **카탈로그 GRANT SQL**을 Databricks 노트북에서 실행
2. Genie Code (✨) → Settings → MCP Servers → "+ Add Server" → `mcp-ai-dev-kit` 선택
3. 필요한 도구만 ON (아래 [도구 선택 가이드](#도구-20개-제한--필요한-것만-켜기) 참조)
...
> 간편 배포(`deploy.sh`)를 사용했다면 이 섹션은 건너뛰고 [Genie Code에서 MCP 서버 연결](#step-7-genie-code에서-mcp-서버-연결)로 이동하세요.
