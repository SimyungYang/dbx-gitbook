...
> **소요 시간**: ~5분 | **핵심**: Databricks Apps에 AI Dev Kit MCP 서버를 배포하기
>
> **간편 배포**: 아래 수동 과정 대신 [Genie Code에 AI Dev Kit 구성하기](../section-3-ai-tools-setup/genie-code-aidevkit)의 `deploy.sh`를 사용하면 3줄로 완료됩니다. 이 페이지는 수동 설정이 필요할 때 참조하세요.
...
> 💡 **화면 참고**: `databricks current-user me` 실행 결과 — 사용자 정보 확인
...
> 💡 **화면 참고**: 앱 생성 결과 — app_url, service_principal 등 출력
...
> 💡 **화면 참고**: 권한 부여 후 확인 — SQL Warehouse Permissions 화면
