...
Smart TV에서 실제로 발생하는 텔레메트리 로그 체계를 기반으로 **10개 카테고리, 17개 테이블, 약 250만 건**의 가상 데이터를 생성합니다.

Smart TV OS의 실제 로깅 인프라(`pmlogd`, Luna Service API, `rdxd`)의 구조를 반영하여, 현실적인 필드명과 값 분포를 가진 데이터를 만듭니다.
...
| 컬럼 | 타입 | 설명 | 예시 값 |
|------|------|------|---------|
| `device_id` | STRING | 고유 디바이스 ID | `STV_KR_000001` |
...
| `os_version` | STRING | Smart TV OS 버전 | `24.10.40`, `23.08.25`, `6.0`, `5.0` |
...
```
10,000개의 Smart TV 디바이스 마스터 데이터를 생성해줘.

조건:
- 테이블: smart_tv.bronze.devices
- device_id: "STV_{region}_{6자리 시퀀스}" 형식 (예: STV_KR_000001)
- model_name: Smart TV 실제 모델 naming convention 따를 것
...
- os_version: "24.10.40"(30%), "23.08.25"(25%), "6.0"(20%), "5.0"(15%), "4.5"(10%)
...
> Smart TV의 `pmlogd` 로그 형식을 기반으로 한 부팅/전원 상태 전환 이벤트.
...
| `device_id` | STRING | FK → devices | `STV_KR_000001` |
...
| `os_version` | STRING | OS 버전 | `24.10.40` |
| `firmware_version` | STRING | 펌웨어 버전 | `03.33.85` |
...
```
50,000건의 Smart TV OS TV 부팅/전원 이벤트 로그를 생성해줘.
...
- os_version, firmware_version은 devices 테이블과 조인하여 사용
...
> Smart TV의 `com.webos.service.utp.broadcast` Luna Service 기반 채널/앱 시청 기록.
...
> Smart TV의 `com.webos.applicationManager` (SAM) Luna Service 기반.
...
- cec_device_name 예시:
  ...
  - stb → "Apple TV 4K", "Chromecast", "Fire TV Stick", "KT IPTV", "SK Btv", "통신사 IPTV"
  - bluray → "블루레이 플레이어 (브랜드 X)", "Sony UBP-X800M2"
...
> Smart TV의 `com.webos.service.connectionmanager` Luna Service 기반.
...
> ACR (Automatic Content Recognition) 시스템. 화면 핑거프린트를 통한 콘텐츠 식별.
...
- ad_brand: 한국 대기업 (제조·통신·유통 등), 미국(Toyota, Apple, Amazon 등)
...
- advertiser_name: 대기업 위주 (제조·통신·유통·플랫폼 등)
...
| `device_id` | STRING | FK → devices (TV) | `STV_KR_000001` |
...
| `iot_device_id` | STRING | IoT 디바이스 ID | `REF_KR_001234` |
...
> Smart TV의 `rdxd` 데몬이 수집하는 크래시/에러 리포트.
...
- 구형 os_version(5.0, 4.5)에서 에러 발생률 2배 높게
