...
> 💡 **화면 참고**: Databricks 워크스페이스 랜딩 페이지
...
> 💡 **화면 참고**: 새 노트북 생성 → 이름 변경 → Serverless 컴퓨트 연결 (초록불)
...
> 💡 **화면 참고**: ✨ 아이콘 클릭 → 사이드 패널 열림 → 하단 Agent 모드 선택
...
> 💡 **화면 참고**: Genie Code가 카탈로그/스키마 생성 코드를 실행하는 화면
...
> 💡 **화면 참고**: Custom Instructions 편집 화면
...
- **[02. 가상 데이터 생성](02-data-generation)** — 17개 테이블, 250만건 Smart TV OS TV 로그 데이터 생성
