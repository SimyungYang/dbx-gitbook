---
title: "SDP 파이프라인 — 데이터 품질과 Medallion Architecture"
---
...
> 💡 **화면 참고**: 초록색 코드 → Accept all 버튼 → 노트북 반영
...
변환 규칙:
1. event_id 기준 중복 제거 (같은 event_id면 최신 session_start만 유지)
...
5. NULL 처리:
   ...
   - hdr_type이 NULL → 'SDR'
6. 추가 컬럼:
   - event_date: CAST(session_start AS DATE) (파티션 키)
...
4. (device_id, timestamp) 기준 중복 제거 — resource_utilization에는 event_id가 없으므로
5. 5분 단위로 집계 (1분 데이터 → 5분 평균):
   - 5분 버킷: FROM_UNIXTIME(UNIX_TIMESTAMP(timestamp) - (UNIX_TIMESTAMP(timestamp) % 300))
...
7. event_date 파티셔닝, device_id Z-ORDER
...
3. devices 테이블과 조인하여 product_line, model_name 추가
4. 추가 컬럼:
   - event_date: DATE 파티션 키
   - is_oom: event_type = 'APP_OOM'
   - is_crash: event_type IN ('APP_CRASH', 'KERNEL_PANIC')
   - is_media_error: event_type = 'MEDIA_PIPELINE_ERROR'
   - device_age_days: DATEDIFF(CAST(timestamp AS DATE), devices.manufacturing_date)
...
- peak_hour_viewers: 같은 시간대(hour) 기준 최대 시청자 수
...
devices 조인으로 product_line, webos_version 추가.
event_date 파티셔닝, device_id Z-ORDER.
```
...
- unique_channel_count: 시청한 고유 채널 수 (다양성 지표)
- -- 디바이스 사용 --
...
- avg_health_score: silver.error_events + silver.system_metrics 기반 직접 계산 (gold.device_health_score 참조하지 않음)
- -- 광고 --
