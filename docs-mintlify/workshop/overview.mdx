---
title: "Genie Code Vibe 워크샵"
---


Databricks 환경에서 **Genie Code**와 **AI Dev Kit**을 활용한 Vibe Coding 실습 워크샵입니다.

실제 제품 IoT 데이터 구조를 기반으로, 자연어 프롬프트만으로 데이터 파이프라인부터 AI 에이전트까지 End-to-End로 구축합니다.
...
| 케이스 | 특징 | 추천 대상 | 섹션 |
|--------|------|----------|------|
| **Smart TV** | 텔레메트리 로그 17테이블, 250만건 | TV/미디어 도메인, 사용자 행동 분석 | [Section 4](section-4-smart-tv-handson/01-setup) |
| **가전 IoT** | 공기청정기+에어컨 센서 17테이블, 220만건 | 가전/IoT 도메인, 센서 데이터 분석 | [Section 5](section-5-appliance-handson/01-setup) |

> **어떤 케이스를 선택해야 하나요?** 두 케이스의 기술 난이도와 실습 내용은 동일합니다. 소속 조직이나 관심 도메인에 맞는 것을 선택하세요. 확실하지 않으면 **Smart TV**를 추천합니다 (범용적인 텔레메트리 패턴).

### 시작 전 확인

| 경험 수준 | 추천 경로 |
|----------|----------|
| **Genie Code 처음** | Section 1 (완전 가이드) → Section 2 → Section 3 → Section 4 or 5 |
| **Genie Code 경험 있음** | Section 3 (환경 구성) → Section 4 or 5 바로 시작 |

### 타임테이블

- [풀데이 (7시간)](timetable-fullday) — Section 1~3 + 핸즈온 전체
- [반나절 (4시간)](timetable-halfday) — 핵심 요약 + 핸즈온 집중
...
| [02. 가상 데이터 생성](section-4-smart-tv-handson/02-data-generation) | Smart TV OS TV 로그 기반 17테이블 250만건 |
...
### Section 5: 가전 IoT (공기청정기 + 에어컨) E2E 핸즈온

| 모듈 | 내용 |
|------|------|
| [01. 환경 설정](section-5-appliance-handson/01-setup) | 카탈로그/스키마 생성, Custom Instructions 설정 |
| [02. 공기청정기 데이터 생성](section-5-appliance-handson/02-air-purifier-data) | PuriCare IoT 센서 8테이블 — PM2.5, 필터, 에너지 |
| [03. 에어컨 데이터 생성](section-5-appliance-handson/03-air-conditioner-data) | DUALCOOL/WHISEN IoT 센서 9테이블 — 온도, 컴프레서, 정비 |
| [04. SDP 파이프라인](section-5-appliance-handson/04-sdp-pipeline) | 가전 IoT 데이터 Medallion Architecture, 데이터 품질 |
| [05. 대시보드 & Genie Space](section-5-appliance-handson/05-dashboard-genie) | 대시보드 3개, Genie Space 3개, 가전 도메인 고도화 |
| [06. 에이전트 개발](section-5-appliance-handson/06-agent-development) | KA + Genie Agent + Supervisor Agent + Lakebase(선택) |
...
| 주제 | URL |
|------|-----|
| Genie Code + MCP 연결 가이드 | [docs.databricks.com/genie-code/mcp](https://docs.databricks.com/aws/en/genie-code/mcp) |
