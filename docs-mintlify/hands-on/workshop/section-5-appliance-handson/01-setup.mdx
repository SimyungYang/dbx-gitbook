...
2. 노트북 이름을 `00_Appliance_Workshop`으로 변경
3. 기본 언어: **Python**
...
> ⚠️ **Stop 버튼 확인**: Genie Code가 작업 중일 때는 입력창 우측에 **빨간색 Stop 버튼**이 표시됩니다. 이 버튼이 보이는 동안은 AI가 아직 코드를 실행하고 있으므로, 다음 프롬프트를 입력하지 마세요.
...
> **참고**: 아래 테이블 수는 **목표값**입니다. 이 시점에서는 아직 테이블이 없으며, [02. 공기청정기 데이터 생성](02-air-purifier-data)과 [03. 에어컨 데이터 생성](03-air-conditioner-data)을 완료하면 생성됩니다.
...
- **[02. 공기청정기 데이터 생성](02-air-purifier-data)** — 공기청정기 IoT 센서 및 DDL 기반 가상 데이터 생성
